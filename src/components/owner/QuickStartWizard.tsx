...
import { UserPlus, Banknote, CheckCircle2, ArrowRight, ArrowLeft, Loader2 } from 'lucide-react';
    ...
    return (
      <div className="card" style={{ textAlign: 'center', padding: '3rem' }}>
        ...
        <p style={{ color: 'var(--text-secondary)', marginBottom: '2rem' }}>
          {formData.name} is now in the system with their payment history.
        </p>
        ...
        <div className="animate-enter">
          ...
          <div style={{ marginTop: '2rem', display: 'flex', justifyContent: 'flex-end' }}>
            <button 
              ...
              onClick={() => setStep(2)}
              disabled={!formData.name || !formData.email || !formData.unit_id}
            >
              Next: Payment History <ArrowRight size={16} />
            </button>
          </div>
        </div>
      )}
      ...
      {step === 2 && (
        <div className="animate-enter">
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
            ...
            <h2 style={{ fontSize: '1.25rem', fontWeight: 700 }}>Record Last 5 Payments</h2>
          </div>

          <p style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', marginBottom: '1.5rem' }}>
            Enter the most recent payments this tenant has made. These will be automatically verified in the system.
          </p>
              ...
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', background: 'var(--bg-surface)', padding: '1rem', borderRadius: '0.5rem', border: '1px solid var(--border)' }}>
                <div className="input-group">
                  ...
                  <input 
                    ...
                    type="number" 
                    value={p.amount || ''}
                    onChange={e => handlePaymentChange(i, 'amount', parseFloat(e.target.value))}
                    placeholder="5000"
                  />
                </div>
                <div className="input-group">
          ...
          {error && (
            <div style={{ marginTop: '1.5rem', background: 'rgba(239,68,68,0.1)', color: '#ef4444', padding: '0.75rem', borderRadius: '0.5rem', fontSize: '0.875rem' }}>
              ⚠️ {error}
            </div>
          )}
